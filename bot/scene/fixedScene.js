...
const updateInfo = new Scene("/обновитьданные",
    async (ctx) => {
        ctx.scene.next();
    ...
    async (ctx) => {
        ...
        try {
            await knex("students").where({ id_vk: parseInt(ctx.message.peer_id) })
                .update({
                    name: ctx.session.name, group: ctx.session.group, year: ctx.session.year,
                    telephone: ctx.session.telephone, email: ctx.session.email
                })
...
const helpScene = new Scene('/помощь',
    async (ctx) => {
        ctx.scene.leave();
        ctx.reply(`
